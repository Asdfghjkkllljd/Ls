...
API_TOKEN = config.CRYPTO_PAY_TOKEN
API_URL = 'https://pay.crypt.bot/api/createCheck'
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(API_URL, headers=headers, json=data) as response:
            if response.status == 200:
                ...
                return result['result']  # Предполагается, что в ответе содержится 'result' с нужной информацией
            else:
                raise Exception(f"Failed to create check: {response.status}, {await response.text()}")
...
async def send_cash_with_error_handling(user_id, user_name, amount, transaction_id):
    try:
        # Если сумма меньше 1.01 доллара, создаем чек и отправляем его пользователю
        if amount < 1.01:
            check = await create_check(amount)
            await send_check_to_user(user_id, check)
        else:
            # В других случаях отправляем деньги напрямую
            await cryptopay.send_cash(user_id, amount, transaction_id)
            await bot.send_message(config.ERROR_CHAT_ID, f"<b> [НОВОЕ ДЕЙСТВИЕ]</b>  \n \n <b>Пользователю {user_name} [{user_id}] была произведена выплата </b> <code>{amount} $ </code> \n \n Баланс после действия: <code> {await cryptopay.balance()} $</code>")
    except Exception as e:
        logging.error(f"Error sending cash: {e}")
        await handle_payment_error(user_id, user_name, amount, str(e))

async def send_check_to_user(user_id, check):
    message = f"Твой чек на {check['amount']} USDT: {check['bot_check_url']}"
