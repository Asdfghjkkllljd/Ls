...
async def lose_notify(msg: str, photo: str, zalupa : float, user_id: str):
    ...
    config.CHANNEL_ID,
    photo,
    f"<b>Вы проиграли.</b>nn"
    "<blockquote>"
    "Попытай свою удачу снова!nЖелаю удачи в следующих ставках!"
    "</blockquote>nn"
    f"<b>• Вам начислен кэшбек {sobaka_str} $ на баланс бота.</b>nn"
    f"Хэш игры: <code> {hash_hex} </code>nn"
    ...
    f'<a href="{config.NEWS_LINK}">Новостной канал</a>',    reply_markup=InlineKeyboardMarkup(
        inline_keyboard=[
...
async def nic_notify(msg: str, photo: str, bid: float, user_id: str):
    ...
    return await bot.send_photo(
        ...
        reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[
                [InlineKeyboardButton(                    "⚡️ Сделать ставку",
                    config.INVOICE_LINK
                )],
            ]
        )
...
async def notify_bid(data: dict, game):= InlineKeyboardMarkup(row_width=1)
    kb.add(
        InlineKeyboardButton(
            "✅ Одобрить",
